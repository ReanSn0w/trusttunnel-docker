...
Текущая реализация автоматически начинает первый ACME-выпуск в фоне после
старта приложения, если заданы `TT_TLS_HOSTNAME` и `TT_ACME_EMAIL` или эти
поля сохранены через панель. Существующая подходящая PEM-пара переиспользуется;
после ошибок применяется повтор с увеличивающейся задержкой. Рабочая пара
остаётся на месте при неудачном обновлении. Панель администрирования пока
использует отдельный TLS reverse proxy. Единый встроенный HTTPS и три источника
сертификата описаны в `plans/shared-admin-vpn-tls.md` и
`plans/automatic-tls-lifecycle.md`; они ещё не реализованы.
