...
ui_login_and_export() {
  ...
  if [ "$bootstrap" = yes ]; then
    curl -fsS -D "$headers" "http://127.0.0.1:$port/bootstrap" -o "$body"
    seed="$(sed -n 's/^Set-Cookie: __Host-tt_csrf_seed=\([^;]*\).*/\1/p' "$headers" | tr -d '\r' | tail -1)"
    csrf="$(sed -n 's/.*name="_csrf" value="\([^"]*\)".*/\1/p' "$body" | tail -1)"
    curl -fsS -o /dev/null -H "Cookie: __Host-tt_csrf_seed=$seed" \
      --data-urlencode "_csrf=$csrf" --data-urlencode 'username=admin' \
      --data-urlencode 'password=Admin-Smoke-9!' "http://127.0.0.1:$port/bootstrap"
  fi
  curl -fsS -D "$headers" -H "Cookie: __Host-tt_csrf_seed=$seed" "http://127.0.0.1:$port/login" -o "$body"
  if [ -z "$seed" ]; then seed="$(sed -n 's/^Set-Cookie: __Host-tt_csrf_seed=\([^;]*\).*/\1/p' "$headers" | tr -d '\r' | tail -1)"; fi
  csrf="$(sed -n 's/.*name="_csrf" value="\([^"]*\)".*/\1/p' "$body" | tail -1)"
  curl -fsS -D "$headers" -o /dev/null -H "Cookie: __Host-tt_csrf_seed=$seed" \
    --data-urlencode "_csrf=$csrf" --data-urlencode 'username=admin' \
    --data-urlencode 'password=Admin-Smoke-9!' "http://127.0.0.1:$port/login"
  session="$(sed -n 's/^Set-Cookie: __Host-tt_session=\([^;]*\).*/\1/p' "$headers" | tr -d '\r' | tail -1)"
  curl -fsS -H "Cookie: __Host-tt_session=$session" "http://127.0.0.1:$port/" -o "$body"
  grep -q 'ready' "$body"
  csrf="$(sed -n 's/.*name="csrf-token" content="\([^"]*\)".*/\1/p' "$body" | tail -1)"
  curl -fsS -H "Cookie: __Host-tt_session=$session" -H "X-CSRF-Token: $csrf" \
    -X POST "http://127.0.0.1:$port/users/1/client/deeplink" | grep -q 'tt://'
}
...
run_platform() {
  ...
  docker run -d --name "$container" --platform "linux/$arch" --read-only \
    ...
    -p "127.0.0.1:$ui_port:8080/tcp" -p "127.0.0.1:$vpn_port:8443/tcp" \
    -p "127.0.0.1:$vpn_port:8443/udp" -v "$volume:/var/lib/trusttunnel" \
    "$image" --ui-listen 0.0.0.0:8080 >/dev/null
...
docker run -d --name "$upgrade_container" --platform linux/amd64 --read-only --tmpfs /tmp:size=16m \
  --cap-drop ALL -p 127.0.0.1:19082:8080/tcp -p 127.0.0.1:19445:8443/tcp \
  -p 127.0.0.1:19445:8443/udp -v "$data_volume:/var/lib/trusttunnel" \
  "$new_image" --ui-listen 0.0.0.0:8080 >/dev/null
...
docker run -d --name "$rollback_container" --platform linux/amd64 --read-only --tmpfs /tmp:size=16m \
  --cap-drop ALL -p 127.0.0.1:19083:8080/tcp -p 127.0.0.1:19446:8443/tcp \
  -p 127.0.0.1:19446:8443/udp -v "$restore_volume:/var/lib/trusttunnel" \
  "$old_image" --ui-listen 0.0.0.0:8080 >/dev/null
